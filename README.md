...
2. **Live widget (TUI only)** — a single tree-branch widget anchored above the editor (`pi-workflow-live`, `aboveEditor` placement):
   - **Collapsed heartbeat** — 2-3 rows showing spinner, workflow entry point and mode (e.g. `spec/strict`), active human-readable stage, node elapsed duration, node-scoped token count, active agent role, and current action (`Ctrl+O 查看最近活动`).
   - **Expanded Activity Tape** — rolling history of recent tool activities (concise Read, Search, Edit, Run), latest output evidence (up to 2 lines), telemetry freshness (age of last update, warning on staleness), cumulative tool calls, next workflow route, and optional diagnostics when stale/failed (`Ctrl+O 折叠`). Full logs remain in underlying subagent/session artifacts.
   The widget refreshes on a 500 ms spinner tick and only re-renders when its render key changes. In RPC mode the same renderer is installed and updated via the UI port; in print/JSON mode no visible widget is mounted (the notifier still instantiates an internal no-op widget and timer per run).
